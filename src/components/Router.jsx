...
export default function Router() {
  ...
  // For BOBTAIL/FLATBED/RABON: only need inspectionType
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {page === 'form' && canEdit() && (
          <div className="space-y-5">
            ...
            {/* For BOBTAIL/FLATBED/RABON: go directly to 20 points, no button needed */}
            {/* For LOADED/EMPTY: Show "Start Inspection" button after unit info is complete */}
            {inspectionTypeSelected && unitInfoFlowComplete && !showInspectionPoints && unitInfo?.inspectionType !== 'BOBTAIL' && unitInfo?.inspectionType !== 'FLATBED' && unitInfo?.inspectionType !== 'RABON' && (
            ...
            {/* Only show inspection components after clicking the button (or directly for BOBTAIL/FLATBED/RABON) */}
            {inspectionTypeSelected && unitInfoFlowComplete && (showInspectionPoints || unitInfo?.inspectionType === 'BOBTAIL' || unitInfo?.inspectionType === 'FLATBED' || unitInfo?.inspectionType === 'RABON') && (
              <>
                <TruckDiagram />
                <InspectionList />
